...
from app.models import Listing,LikedListing
from .forms import *
...
def login_page(request):
    if request.method=="POST":
        ...
        if login_form.is_valid():
            ...
            if user is not None:
                login(request,user)
                messages.success(request,f"sucessfully login  as {username}")
                return redirect('login_home')
            else:
                messages.error(request,f"unable to login try to login")
        else:
            messages.error(request, f"unable to login try to login")
...
class ProfileView(View):
    def get(self,request):
        ...
        liked_listing=LikedListing.objects.filter(profile=request.user.profile).all()
    ...
    def post(self, request):
        user_listings = Listing.objects.filter(seller=request.user.profile)
        liked_listing = LikedListing.objects.filter(
